...
size_t skip_no_letters(const char* str, const size_t len);
...
int main()
{
    ...
    FILE * input_file = open_file(); // открытие файла
...
int cmpstrings(const void* a, const void* b)
{
    ...
    size_t i = 0;
    size_t j = 0;
    while (i < arg1_len && j < arg2_len)
    {
        i += skip_no_letters(arg1_start, arg1_len);
        j += skip_no_letters(arg2_start, arg2_len);
        if (arg1_start[arg1_len - 1 - i] > arg2_start[arg2_len - 1 - j])
        {
            return 1;
        } else if (arg1_start[arg1_len - 1 - i] < arg2_start[arg2_len - 1 - j]) 
        {
            return -1;
        }
        i++;
